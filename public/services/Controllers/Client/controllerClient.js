    ...
    if (user) {
        ...
        onValue(RoleData, (snapshotRole) => {
            ...
            if (role == "client"){
                ...
                onValue(PrenomData, (snapshotPrenom) => {
                    const Prenom = snapshotPrenom.val();
                    console.log(Prenom);

                    document.getElementById("UserPrenom").innerHTML = Prenom;
                })

                const SexeData = ref(database, 'users/' + user.uid + '/sexe');
                onValue(SexeData, (snapshotSexe) => {
                    const Sexe = snapshotSexe.val();
                    console.log(Sexe);

                    switch(Sexe){
                        case "homme":
                            document.getElementById("UserSexe").innerHTML = "Mr.";
                            break;
                        case "femme":
                            document.getElementById("UserSexe").innerHTML = "Mme.";
                            break;
                        case "autre":
                            document.getElementById("UserSexe").innerHTML = "Nb.";
                            break;
                        default:
                            document.getElementById("UserSexe").innerHTML = "Non-Renseigné.";
                    }
                })
            }
            else{
